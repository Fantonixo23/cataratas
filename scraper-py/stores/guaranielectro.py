...
STORE_ID = "guaranielectro"

def scrape(query: str) -> list[dict]:
    url = f"https://guaranielectro.com.py/?s={query}&post_type=product"
    try:
        soup = fetch_html(url)
    except Exception:
        return []
    ...
    for card in soup.select(".product, li.product, .product-item, article"):
        ...
        if href and not href.startswith("http"):
            href = f"https://guaranielectro.com.py{href}" if href.startswith("/") else f"https://guaranielectro.com.py/{href}"
        ...
        ext_id_match = re.search(r"/product/(\d+)", href) or re.search(r"p=(\d+)", href) or re.search(r"-(\d+)$", href)
