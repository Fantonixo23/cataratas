...
def scrape(query: str) -> list[dict]:
    url = f"https://www.bristol.com.py/catalogo?q={query}"
    try:
        soup = fetch_html(url)
    except Exception:
        return []
    ...
    products = []
    for card in soup.select(".product-card, article, .item, [class*=product], .card-produto"):
        name_el = card.select_one("h3 a, h4 a, .product-name a, [class*=titulo] a, a[class*=product]")
        img_el = card.select_one("img")
        price_el = card.select_one("[class*=price], .precio, [class*=preco], .valor, .preco-promocional")

        if not name_el:
            name_el = card.select_one("a[href*='/produto/'], a[href*='/producto/']")
            if not name_el:
                continue
        ...
        if href and not href.startswith("http"):
            href = f"https://www.bristol.com.py{href}" if href.startswith("/") else f"https://www.bristol.com.py/{href}"
